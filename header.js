...
const handleClientNav = () => {
  ...
  function createElem(elemType, elemOptions, elemText) {
    ...
    const allPanels = document.querySelectorAll('.ob-header-nav-desktop-container');
    ...
    const addClickEvents = () => {
        ...
        topMenuItems.forEach((menuItem) => {
            ...
            menuItem.addEventListener('click', (e) => {
                ...
                if (foundParent.classList.contains('is-active')) {
                    ...
                    document.querySelector(`#ob-${panelToOpen}-panel`).classList.toggle('is-open');
                } else {
        ...
        mobileMenuItems.forEach((menuItem) => {
            ...
            menuItem.addEventListener('click', (e) => {
                ...
                if (!eventTarget.parentNode.parentNode.classList.contains('ob-tier-3') && eventTarget.parentNode.parentNode.classList.contains('ob-mobile-has-submenu')) {
                    // ---- Prevent the default action
                    e.preventDefault();
                    // ---- Stop event propagation
                    e.stopPropagation();
                    if (eventTarget.parentNode.parentNode.classList.contains('ob-mobile-header-nav-list-item')) {
                    ...
                    if (eventTarget.parentNode.parentNode.classList.contains('ob-mobile-header-nav-list-submenu-item')) {
                        const otherMenus = getSiblings(eventTarget.parentNode.parentNode);
                        otherMenus.forEach((menu) => {
                            menu.classList.remove('is-open');
                        });
        ...
        burgerElement.addEventListener('click', (e) => {
            ...
            document.querySelector('.ob-mobile-header-nav-container').classList.toggle('is-open');
        });
    };
